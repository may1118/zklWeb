...
Tree.prototype.render = function (noCallbacks) {
  ...
  // remove items first
    for (var ii = 0; ii < this.keysToRemove.length; ++ii) {
        ...
        if (node) {
            var nodeVal = node.outerHTML;
            var cdbm = $(nodeVal).attr("data-value");
            var index = arrCdbm.indexOf(cdbm);
            if (index > -1) {
                arrCdbm.splice(index, 1);
            }
            var b = arrCdbm.join(",");
            $("#hdUser").val(b);
  ...
  // now add items
    ...
    var html = selectedNode;//option.value
...
exports.createSelected = function (option, disableRemoval, showSectionOnSelected) {
  ...
  if (showSectionOnSelected) {
      var sectionSpan = exports.createNode('span', { class: 'section-name', text: option.text });
      arrCdbm.push(option.value);
      var b = arrCdbm.join(",");
      $("#hdUser").val(b);
